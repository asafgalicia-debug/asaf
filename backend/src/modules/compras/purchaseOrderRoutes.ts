...
import { createPurchaseOrder, listPurchaseOrders } from './purchaseOrderService.js';
...
export function createPurchaseOrderRoutes(): Router {
  ...
  router.post('/', authenticate, tenant, authorize('usuarios.editar'), async (req, res, next) => { try { const s = scope(req); const parsed = schema.safeParse(req.body); if (!parsed.success) throw new AppError({ code: 'VALIDATION_ERROR', message: 'Invalid purchase order data', friendlyMessage: 'Revisa proveedor, producto, cantidad y costo unitario.', statusCode: 400 }); const row = await createPurchaseOrder({ ...parsed.data, companyId: s.companyId, branchId: s.branchId }); await logAuditEvent({ userId: s.userId, companyId: s.companyId, branchId: s.branchId, action: 'CREATE', module: 'compras', entityId: row.id, details: { total: row.total }, ipAddress: req.ip }); res.status(201).json({ ok: true, data: row }); } catch (error) { next(error); } });
  return router;
}